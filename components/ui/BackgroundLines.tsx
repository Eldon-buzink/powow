...
export const BackgroundLines = ({
  ...
  return (
    <div
      className={cn(
        "relative h-[20rem] md:h-[32rem] w-full bg-white overflow-hidden",
        className
      )}
    >
      <SVG svgOptions={svgOptions} />
      <div className="relative z-10 flex flex-col items-center justify-center h-full w-full">{children}</div>
    </div>
  );
};
...
const SVG = ({
  ...
  // Render more lines for frequency
  const allPaths = Array(3).fill(paths).flat();
  const allColors = Array(3).fill(colors).flat();
      ...
      >
        {allPaths.map((path, idx) => (
          <motion.path
            ...
            key={`path-frequent-${idx}`}
            style={{ opacity: 0.85 }}
          />
        ))}
      </motion.svg>
      {/* Fade out at the bottom with a white gradient overlay */}
